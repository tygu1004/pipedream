...
#parameter_sizes[i][j] => i+1 ~ j+2 cum_par_size

def evolute(gene):	#gene => [ [ [machine_type, allocate_num, fr, to], ...], [slowest, slowest_stage_idx]]
	slowest_stage_idx = gene[1][1]
	gene_info = gene[0]

	num_layers_in_stage = gene_info[slowest_stage_idx][3] - gene_info[slowest_stage_idx][2]
	max_rate = 0.1	#max_rate have to < 0.5

	l = randint(0, round(num_layers_in_stage * max_rate))	#how many layer will be moved

	if (slowest_stage_idx-1 >= 0 ) and (gene_info[slowest_stage_idx][2] + l < gene_info[slowest_stage_idx][3]):	#move to prev stage
		gene_info[slowest_stage_idx-1][3] += l
		gene_info[slowest_stage_idx][2] += l

	l = randint(0, round(num_layers_in_stage * max_rate))	#how many layer will be moved

	if (slowest_stage_idx+1 < len(gene_info)) and (gene_info[slowest_stage_idx][3] - l > gene_info[slowest_stage_idx][2]):	#move to next stage
		gene_info[slowest_stage_idx+1][2] -= l
		gene_info[slowest_stage_idx][3] -= l

	return gene_info

def gene_compute_tiems(gene_info, compute_times_1, compute_times_2, parameter_sizes, bandwidth, output_activation_sizes):
	result = [] 	#comptime
	for i, stage in enumerate(gene_info):
		tmp = []
		m_type = stage[0]
		m_num = stage[1]
		fr_layer = stage[2]
		...
		parameter_size = parameter_sizes[fr_layer-1][to_layer-2]
		activation_size = output_activation_sizes[to_layer-2]

		if parameter_size != None:
			dp_communication_time = (4 * (m_num-1) * parameter_size) / (bandwidth * m_num)
		else:
			dp_communication_time = 0

		if i != len(gene_info)-1:
			stage_communication_time = (2.0 * activation_size) / bandwidth
		else:
			stage_communication_time = 0

		if m_type == 0:
			if fr_layer != len(compute_times_1)-1 :
				compute_time = compute_times_1[fr_layer-1][to_layer-2]
			else:
				compute_time = compute_times_1[fr_layer-2][to_layer-2]
		else:
			if fr_layer != len(compute_times_2)-1 :
				compute_time = compute_times_2[fr_layer-1][to_layer-2]
			else:
				compute_time = compute_times_2[fr_layer-2][to_layer-2]

		'''
		if compute_time == None or dp_communication_time == None or stage_communication_time == None:
			print("fr_layer",fr_layer, "to_layer", to_layer)
			print(compute_time, dp_communication_time, stage_communication_time)
			print(stage)
		'''
		result.append(max(compute_time, dp_communication_time, stage_communication_time))

	slowest = max(result)
	slowest_idx = result.index(slowest)	
	return [slowest, slowest_idx]

def make_random_gene(num_machines_1, num_machines_2, total_layer):
	m1 = num_machines_1
	m2 = num_machines_2

	stage_num = randint(2, num_machines_1 + num_machines_2)
	...
	for i in range(0, stage_num):
		if i == stage_num-1:
			if m1 == num_machines_1:
				gene_info.append([0, 1])
				m1 -= 1
				continue
			elif m2 == num_machines_2:
				gene_info.append([1, 1])
				m2 -= 1
				continue

		if m1 > 0 and m2 > 0:
			machine_type = randint(0, 1)
			if machine_type == 0:
				gene_info.append([0, 1])
				m1 -= 1
			else:
	...
	while m1 != 0 or m2 != 0:
		for i in range(0, stage_num):
			if gene_info[i][0] == 0 and m1 > 0:
				allocate_num = randint(1, m1)
				gene_info[i][1] += allocate_num
				m1 -= allocate_num
			elif gene_info[i][0] == 1 and m2 > 0:
				allocate_num = randint(1, m2)
				gene_info[i][1] += allocate_num
				m2 -= allocate_num
	...
	while len(split_info) != stage_num-1:
		s = randint(1, total_layer-1)
		if (s not in split_info) and (s+1 not in split_info) and (s-1 not in split_info):
			split_info.append(s)
	
	split_info.sort()
	#print("split_info", split_info)
	
	for i in range(0, len(split_info)+1):
		if i == 0:
			gene_info[i].append(1)
			gene_info[i].append(split_info[i])
		elif i == len(split_info):
			gene_info[i].append(gene_info[i-1][3]+1)
			gene_info[-1].append(total_layer+1)
		else:
			gene_info[i].append(gene_info[i-1][3]+1)
			gene_info[i].append(split_info[i])
	

	return gene_info 		#[[machine_type, allocate_num, fr, to], ...]

def evolution_algorithm(compute_times_1, compute_times_2, activation_sizes, parameter_sizes,
						 output_activation_sizes, all_predecessor_ids,
						 num_machines_1, num_machines_2, num_machines_within_machine,
						 bandwidth, final_level=True):
	
	total_layer = len(compute_times_1[0])
	...
	for iteration in range(0, generation):
		if len(gene) == 0:
			for i in range(0, gene_num):
				tmp = []
				new_gene = make_random_gene(num_machines_1, num_machines_2, total_layer)
				tmp.append(new_gene)
				tmp_stage_time = gene_compute_tiems(new_gene, compute_times_1, compute_times_2, parameter_sizes,
													bandwidth, output_activation_sizes)
				tmp.append(tmp_stage_time)		#[[[machine_type, allocate_num, fr, to], ...], [slowest, slowest_stage_idx]]
				gene.append(tmp)
		else:
			while len(gene) < (gene_num - mutation):
				tmp_gene = []
				for i in gene:
					tmp = []
					new_gene = evolute(i)
					...
					tmp_stage_time = gene_compute_tiems(new_gene, compute_times_1, compute_times_2, parameter_sizes,
														bandwidth, output_activation_sizes)
					tmp.append(tmp_stage_time)
					tmp_gene.append(tmp)
					if(len(tmp_gene) == gene_num - mutation - len(gene)):
						break
				gene.extend(tmp_gene)

			for i in range(0, mutation):	#mutation
				tmp = []
				new_gene = make_random_gene(num_machines_1, num_machines_2, total_layer)
				tmp.append(new_gene)
				tmp_stage_time = gene_compute_tiems(new_gene, compute_times_1, compute_times_2, parameter_sizes,
		...
		gene = sorted(gene, key=lambda g: g[1][0])

		
		if iteration != generation-1:
			gene = gene[:select_num]
	'''
	for i, k in enumerate(gene):
		print(i, ":", k)
	'''
	split = []
	for st in gene[0][0]:
...
def compute_partitioning(compute_times, activation_sizes, parameter_sizes,
						 output_activation_sizes, all_predecessor_ids,
						 num_machines, bandwidth, final_level=True):
	A = []

	'''
	for i in output_activation_sizes:
		print(i)
	print(len(output_activation_sizes))

	for i in activation_sizes:
		print(i)

	for x in np.array(parameter_sizes):
		print(x)
		print(len(x))
	'''
	for i in range(len(compute_times)):
		...
		for j in range(len(compute_times[0])):
			...
			for m in range(num_machines):
				row_row_A.append((None, None, None))	#(fr, to, #machine)
		...
		for j in range(i, len(compute_times[0])):
			...
			for m in range(max_m):
				...
				if cum_compute_time is None:
					...
					A[i][j][m] = (sum([cum_compute_time, data_parallel_communication_time]) / (m+1), None, (m+1))		#A[i][j][m] : (stage(i-th ~ j-th) time with m+1 machines, none, m+1)
			...
			for j in range(i+1, len(compute_times[0])):
				...
				for k in all_predecessor_ids[j]:
					...
					for m_prime in range(1, max_m_prime):
						input_transfer_time = (2.0 * output_activation_sizes[k]) / (bandwidth * m_prime)
						...
						last_stage_time = compute_times[k+1][j]	#k+1 ~ j sum of compute time
						...
						if use_memory_constraint and stashed_data_size > memory_size:
							continue
						'''
						last_stage_time = sum([last_stage_time, ((4 * (m_prime - 1) * last_stage_parameter_size) / (bandwidth * m_prime))])
	...
	for i, Ai in enumerate(A):
		for j, Aij in enumerate(Ai):
			for m, Aijm in enumerate(Aij):
				if Aijm != (None, None, None):
					print(Aijm, " : ", i, j, m)
	'''
	#print(np.array(A))
	print("A shape : ", np.array(A).shape)
	#for tmp in np.array(A):
...
def main(all_num_machines_1, all_num_machines_2, profile_filename_1, profile_filename_2, network_bandwidths, memory_size,
	...
	gr = graph.Graph.from_str(open(profile_filename_1, 'r').read())
	...
	for i in range(len(states)+1):
		...
		compute_times_1.append(compute_times_row)
		activation_sizes.append(activation_sizes_row)
		parameter_sizes.append(parameter_sizes_row)

	###############################
	gr2 = graph.Graph.from_str(open(profile_filename_2, 'r').read())
	...
	for source in sources:
		if source.node_desc.startswith("Input"):
			...
			source.parameter_size = 0.0
			nodes_to_remove[source] = []
			for out_node in gr2.edges[source.node_id]:
				nodes_to_remove[source].append(out_node)
			gr2.remove_node(source)
	...
	for sink in sinks:
		if sink.node_desc.startswith("__getitem__"):
			gr2.remove_node(sink)

	antichain_gr2 = gr2.antichain_dag()
	states = antichain_gr2.topological_sort()
	if verbose:
		print("Total number of states: %d" % len(states))
	states_indices = {}
	for i in range(len(states)):
	...
	for i in range(len(states)):
		for antichain_node in states[i].antichain:
			states[i].output_activation_size += gr2.nodes[antichain_node].activation_size

	for i in range(len(states)):
		antichain = states[i].antichain
		all_predecessors = gr2.all_predecessors(antichain)
		states[i].compute_time = 0.0
		states[i].activation_size = 0.0
		states[i].parameter_size = 0.0
		for predecessor in all_predecessors:
	...
	output_activation_sizes = [state.output_activation_size for state in states]
	all_predecessor_ids = [[states_indices[predecessor] for predecessor in
							antichain_gr2.predecessors(states[i].node_id)]
						   for i in range(len(states))]
		...
		for j in range(len(states)):
			if i == 0:
				...
				if j > (i-1):
					...
					activation_sizes_row.append(None)
					parameter_sizes_row.append(None)
		compute_times_2.append(compute_times_row)
		activation_sizes.append(activation_sizes_row)
		parameter_sizes.append(parameter_sizes_row)
	...
	for num_machines, network_bandwidth in zip(all_num_machines_1 + all_num_machines_2, network_bandwidths):
	...
	while i >= 0:
		...
		for (start, end) in splits:
			...
			partial_splits = evolution_algorithm(compute_times_1, compute_times_2, activation_sizes, parameter_sizes,
						 output_activation_sizes, all_predecessor_ids,
						 all_num_machines_1[0], all_num_machines_2[0], 1,
						 bandwidth)
			'''
			partial_splits = \
	...
	if output_directory is not None:
		total_num_machines = all_num_machines_1[0] + all_num_machines_2[0]
...
if __name__ == '__main__':
	...
	parser.add_argument('-n1', "--all_num_machines_1", nargs='+', type=int,
						help="Number of machines available")
	parser.add_argument('-n2', "--all_num_machines_2", nargs='+', type=int,
						help="Number of machines available")
	
	parser.add_argument('-f1', "--profile_filename_1", required=True,
						help="Profile filename")
	parser.add_argument('-f2', "--profile_filename_2", required=True,
						help="Profile filename")
	...
	all_num_machines_1 = args["all_num_machines_1"]
	all_num_machines_2 = args["all_num_machines_2"]
	profile_filename_1 = args["profile_filename_1"]
	profile_filename_2 = args["profile_filename_2"]
	network_bandwidths = args["network_bandwidths"]
	#assert(len(all_num_machines_1) + len(all_num_machines_2) == len(network_bandwidths))
	...
	main(all_num_machines_1, all_num_machines_2, profile_filename_1, profile_filename_2, network_bandwidths, memory_size,
		 straight_pipeline, use_memory_constraint, use_fewer_machines,
		 activation_compression_ratio, output_directory,
		 verbose=True)
